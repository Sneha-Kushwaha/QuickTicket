...
const MovieCard = ({movie}) => {
  ...
  return (
    <div className='flex flex-col justify-between p-3 bg-grey-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-[220px]'>

        <img onClick={()=> {navigate(`/movies/${movie._id}`); scrollTo(0, 0)}}
         src={movie.backdrop_path} alt="" className="rounded-lg h-60 w-full object-cover object-center cursor-pointer" />

        <p className='font-semibold mt-2 truncate'>{movie.title}</p>

        <p className='text-sm text-gray-400 mt-2 whitespace-nowrap'>
